...
// Augmented w velocity model for elevation control with integral action, two states, one input, one output
struct wVelocityModel : public Model <2,1,1>
{
    wVelocityModel(float Zw, float Z_strokeAmplitude, float m, float Ts)
    {   
        // Define the system matrix
        A = {Zw/m, 0, -Ts, 1.0000};

        // Define the input matrix
        B = {Z_strokeAmplitude/m, 0;
